...
int GeneralRateModelDG::multiplexInitialConditions(const cadet::ParameterId& pId, unsigned int adDirection, double adValue)
{
	if (_singleBinding)
	{
		if ((pId.name == hashString("INIT_CP")) && (pId.section == SectionIndep) && (pId.boundState == BoundStateIndep) && (pId.particleType == ParTypeIndep) && (pId.component != CompIndep) && (pId.reaction == ReactionIndep))
...
int GeneralRateModelDG::multiplexInitialConditions(const cadet::ParameterId& pId, double val, bool checkSens)
{
	if (_singleBinding)
	{
		if ((pId.name == hashString("INIT_CP")) && (pId.section == SectionIndep) && (pId.boundState == BoundStateIndep) && (pId.particleType == ParTypeIndep) && (pId.component != CompIndep) && (pId.reaction == ReactionIndep))
...
void GeneralRateModelDG::applyInitialCondition(const SimulationState& simState) const
{
	Indexer idxr(_disc);
...
void GeneralRateModelDG::readInitialCondition(IParameterProvider& paramProvider)
{
	_initState.clear();
	_initStateDot.clear();
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void GeneralRateModelDG::consistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
	...
	for (unsigned int type = 0; type < _disc.nParType; ++type)
	{
...
#endif
		{
			...
			linalg::DenseMatrixView jacobianMatrix(jacobianMem, _jacPdisc[type * _disc.nPoints + pblk].innerIndexPtr(), probSize, probSize);
			const parts::cell::CellParameters cellResParams = makeCellResidualParams(type, mask.mask + _disc.nComp);
 ...
 * @param [in,out] vecStateYdot On entry, residual without taking time derivatives into account. On exit, consistent state time derivatives.
 */
void GeneralRateModelDG::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY, double* const vecStateYdot, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
...
#endif
	{
		...
		for (unsigned int j = 0; j < _disc.nParPoints[type]; ++j)
		{
			...
			// Get iterators to beginning of solid phase
			linalg::BandedEigenSparseRowIterator jacSolidOrig(_jacP[pblk], j * static_cast<unsigned int>(idxr.strideParShell(type)) + static_cast<unsigned int>(idxr.strideParLiquid()));
		...
		// Factorize
		_parSolver[type * _disc.nPoints + par].factorize(mat);
...
		Mixed FV-DG implementation			

int GeneralRateModelDGFV::multiplexInitialConditions(const cadet::ParameterId& pId, unsigned int adDirection, double adValue)
{
	if (_singleBinding)
	{
		if ((pId.name == hashString("INIT_CP")) && (pId.section == SectionIndep) && (pId.boundState == BoundStateIndep) && (pId.particleType == ParTypeIndep) && (pId.component != CompIndep) && (pId.reaction == ReactionIndep))
...
int GeneralRateModelDGFV::multiplexInitialConditions(const cadet::ParameterId& pId, double val, bool checkSens)
{
	if (_singleBinding)
	{
		if ((pId.name == hashString("INIT_CP")) && (pId.section == SectionIndep) && (pId.boundState == BoundStateIndep) && (pId.particleType == ParTypeIndep) && (pId.component != CompIndep) && (pId.reaction == ReactionIndep))
...
void GeneralRateModelDGFV::applyInitialCondition(const SimulationState& simState) const
{
	Indexer idxr(_disc);
...
void GeneralRateModelDGFV::readInitialCondition(IParameterProvider& paramProvider)
{
	_initState.clear();
	_initStateDot.clear();
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void GeneralRateModelDGFV::consistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
 ...
 * @param [in,out] vecStateYdot On entry, residual without taking time derivatives into account. On exit, consistent state time derivatives.
 */
void GeneralRateModelDGFV::consistentInitialTimeDerivative(const SimulationTime& simTime, double const* vecStateY, double* const vecStateYdot, util::ThreadLocalStorage& threadLocalMem)
{
	BENCH_SCOPE(_timerConsistentInit);
 ...
 * @param [in] errorTol Error tolerance for algebraic equations
 */
void GeneralRateModelDGFV::leanConsistentInitialState(const SimulationTime& simTime, double* const vecStateY, const AdJacobianParams& adJac, double errorTol, util::ThreadLocalStorage& threadLocalMem)
{
	if (isSectionDependent(_parDiffusionMode) || isSectionDependent(_parSurfDiffusionMode))
		LOG(Warning) << "Lean consistent initialization is not appropriate for section-dependent pore and surface diffusion";
 ...
 * @param [in] res On entry, residual without taking time derivatives into account. The data is overwritten during execution of the function.
 */
void GeneralRateModelDGFV::leanConsistentInitialTimeDerivative(double t, double const* const vecStateY, double* const vecStateYdot, double* const res, util::ThreadLocalStorage& threadLocalMem)
{
	// @TODO?
	//if (isSectionDependent(_parDiffusionMode) || isSectionDependent(_parSurfDiffusionMode))
	//	LOG(Warning) << "Lean consistent initialization is not appropriate for section-dependent pore and surface diffusion";
...
void GeneralRateModelDGFV::initializeSensitivityStates(const std::vector<double*>& vecSensY) const
{
	Indexer idxr(_disc);
	for (std::size_t param = 0; param < vecSensY.size(); ++param)
	{
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void GeneralRateModelDGFV::consistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
	std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	// @TODO?
//	BENCH_SCOPE(_timerConsistentInit);
 ...
 * @todo Decrease amount of allocated memory by partially using temporary vectors (state and Schur complement)
 */
void GeneralRateModelDGFV::leanConsistentInitialSensitivity(const SimulationTime& simTime, const ConstSimulationState& simState,
	std::vector<double*>& vecSensY, std::vector<double*>& vecSensYdot, active const* const adRes, util::ThreadLocalStorage& threadLocalMem)
{
	// @TODO?
 ...
 * @param [in] idxr Indexer
 */
void GeneralRateModelDGFV::solveForFluxes(double* const vecState, const Indexer& idxr) const
{
	// We have j_f - k_f * (c - c_p) == 0
	// Thus, jacFC contains -k_f and jacFP +k_f.
	// We just need to subtract both -k_f * c and k_f * c_p to get j_f == k_f * (c - c_p)
